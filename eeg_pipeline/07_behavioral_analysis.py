...
def process_subject(subject_id: str):
    ...
    """
    print(f"Running behavioral analysis for sub-{subject_id}...")
    
    # Define Input/Output paths using paths.py
    # Change "preprocessing" to whichever folder holds the output of the prior step
    input_dir = paths.OUTPUT_DIR / "preprocessing" 
    output_dir = paths.OUTPUT_DIR / "behavioral_analysis"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Example logic demonstrating how to load data and use your functions:
    # event_file = input_dir / f"sub-{subject_id}_events.csv"
    # if not event_file.exists():
    #     print(f"Warning: Data for sub-{subject_id} not found. Skipping.")
    #     return
    # 
    ...
    #
    # t_matrix = compute_markov_transition_matrix(choices)
    # predictability = compute_predictability(t_matrix)
    # strategy_results = classify_strategy(choices, outcomes)
    # 
    # np.save(output_dir / f"sub-{subject_id}_markov_matrix.npy", t_matrix)
    # ... save other metrics as needed ...

    print(f"Finished behavioral analysis for sub-{subject_id}.")
