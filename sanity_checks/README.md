...
|------|--------|-----------|
| 00 | `sc_00_downsample.py` | Downsampling erfolgreich, Sampling-Rate korrekt, Datengröße reduziert |
| 01 | `sc_01_split_players.py` | Spieler aufgeteilt, Kanäle korrekt zugeordnet, Kanal-Typen gesetzt |
| 02 | `sc_02_rename_montage.py` | Kanäle korrekt umbenannt (BioSemi → 10-20), Montage gesetzt |
| 03 | `sc_03_bad_channels_detect.py` | Bad-Channels identifiziert, QC-Reports erstellt |
| 04 | `sc_04_interpolate.py` | Interpolation durchgeführt, Bad-Listen geleert |
| 05 | `sc_05_filter.py` | Bandpass-Filter angewendet (1-40 Hz), PSD-Reduktion sichtbar |
| 06 | `sc_06_ica.py` | ICA Komponenten extrahiert, EOG-Artefakte erkannt |
| 07 | `sc_07_epoch.py` | Epochs erstellt, Event-Typen korrekt, Zeit-Fenster passen |

## Verwendung

### Einzelnen Sanity Check ausführen
```bash
python sanity_checks/sc_00_downsample.py
```

### Alle Sanity Checks in Folge (optional, können später in Master-Pipeline integriert werden)
```bash
for script in sc_*.py; do python $script; done
```

## Ausgaben

- **Console Output**: Detaillierte Überprüfungs-Ergebnisse mit ✓ (bestanden) und ⚠/ERROR (Warnung/Fehler)
- **QC-Reports**: TSV-Dateien und PNG-Plots werden in `data/qc/` gespeichert
