"""
Sanity Check Visualization for Step 07: Epoching
...
def get_subjects(subject_str):
    if subject_str:
        return [s.strip().zfill(2) for s in subject_str.split(",")]
    return [str(s).zfill(2) for s in list(config.SUBJECTS)[:2]]
...
def plot_event_distribution(epochs, subject_id, person, output_dir):
    ...
    plot_path = output_dir / f"sub-{subject_id}_{person}_epoch_event_distribution.png"
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  ✓ Event distribution saved: {plot_path.name}")
...
def plot_example_epochs(epochs, subject_id, person, output_dir):
    ...
    fig.tight_layout()
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_path = output_dir / f"sub-{subject_id}_{person}_epoch_examples.png"
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  ✓ Example epochs saved: {plot_path.name}")
...
def plot_psd_comparison(raw, epochs, subject_id, person, output_dir):
    ...
    raw_eeg.plot_psd(fmax=40, ax=axes[0], show=False, color="#1f77b4", picks='eeg')
    ...
    # AFTER: Epoched data
    epochs_eeg.plot_psd(fmax=40, ax=axes[1], show=False, color="#ff7f0e", picks='eeg')
    axes[1].set_facecolor("#fff5e6")
    axes[1].set_title(f"AFTER — Epoched Data\n{len(epochs_eeg)} epochs × {len(epochs_eeg.ch_names)} channels",
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_path = output_dir / f"sub-{subject_id}_{person}_epoch_psd_comparison.png"
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
"""
    ...
    fig.tight_layout()
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_path = output_dir / f"sub-{subject_id}_{person}_epoch_statistics.png"
    fig.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  ✓ Statistics summary saved: {plot_path.name}")
...
def process_subject(subject_id):
    ...
    for person in ["P1", "P2"]:
        ...
        if not raw_path.exists() or not epoch_path.exists():
            print(f"    ⚠ Skipping: Missing input/output files")
            continue
