...
class SanityCheckCollector:
    ...
    def __init__(self, step_name: str):
        ...
        self.results: Dict[str, Dict[str, List[str]]] = {}  # {subject: {person: [messages]}}
        self.errors = []
        self.warnings = []

    def add_result(self, subject_id: str, person: str, status: str, message: str):
        """Add a result message. status in ('✓', '⚠', 'ERROR')."""
        ...
        with open(output_path, "w", newline="") as f:
            ...
            for subject_id in sorted(self.results.keys()):
                for person in sorted(self.results[subject_id].keys()):
                    for msg in self.results[subject_id][person]:
                        # Extract status and message from formatted string
                        msg_clean = msg.strip()
                        status = msg_clean.split()[0]
                        message = " ".join(msg_clean.split()[1:])
                        writer.writerow([self.step_name, subject_id, person, status, message])
...
def compare_amplitudes(
    ...
    std_before = np.std(data_before) * 1e6  # Convert to µV
